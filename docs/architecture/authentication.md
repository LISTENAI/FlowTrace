...
同邮箱人员时自动创建 Person。提供方没有返回可信邮箱时阻止首次建档，并提示
成员确认企业账号资料和授权；系统绝不按姓名猜测，更不允许用户从全员目录中
自行认领。邮箱已被另一登录身份占用等歧义会阻止登录，留给管理员核对数据。

升级前已经建立绑定的外部账号即使暂时取不到邮箱仍可继续使用，避免身份提供方
短时降级导致既有成员被锁在系统外；但系统不会再为新的无邮箱身份创建占位人员。
