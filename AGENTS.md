...
`docs/product/requirements-v0.2.md`；AI 接入阶段的增量与覆盖规则位于
`docs/product/ai-integration-v0.2.md`；身份接入增量位于
...
人员工作台与零碎待办增量位于
`docs/product/personal-work-v0.5.md`。可信记录与交付协作增量位于
`docs/product/reliable-delivery-v0.6.md`。
...
- 身份认证不可关闭；一个部署只启用一种登录适配器。外部账号按可信邮箱自动
  创建或关联人员，不按姓名猜测，也不允许用户自行认领人员身份。
- Agent 写入必须携带 `source=agent` 及调用方标识，并走同一业务服务；运行时能
  准确提供模型标识与版本时还应记录，未知时留空，不得猜测。
- 需求、Bug 等可读 ID 一经创建不可因改名或移动版本而变化。
- MCP 必须在不依赖 Skill 的情况下提供自描述、可靠的单项读写能力；Skill
  只负责项目管理方法与调用策略，不能成为接口正确性的补丁。
...
Web 支持跟随系统、浅色和深色三种外观偏好，统一由 `state/theme.ts` 控制。
新增或修改界面时必须同时检查明暗主题，不能只依赖浏览器自动反色。

空库启动时可自动创建基础项目节奏，但正式运行不得默认填充演示项目。演示
数据仅在显式设置 `FLOWTRACE_SEED_DEMO=true` 且项目库为空时注入一次；不得
在后续启动时补回用户已删除的演示内容。开发 Compose 可以显式开启该选项。

正式交付物是一个同时提供 Web 与 API 的应用镜像，容器内部监听 3100 端口，
并连接独立的 PostgreSQL 实例。生产部署规则见
`docs/architecture/deployment.md`。正式环境必须明确配置登录适配器，不能以
匿名模式运行；本地账号模式也必须完成唯一所有者初始化并关闭后续注册。
